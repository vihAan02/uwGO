...
WLU manual form ─────────────▶ CourseMeeting[]  (university: "WLU")
                     ...
                     buildWeekPlan({ meetings, home, mondayISO, config }, provider)
                       ...
                       ├─ 2. buildItinerary   the chain of legs those decisions imply
...
- `transitions.ts`: `buildItinerary(classes, home, goHomeAfter, dateISO)` returns the day's `LegSpec[]` — a **chain**, not a set of independent hops. Each leg starts where the previous one ended. A gap in `goHomeAfter` becomes two legs (class→home, home→next class) instead of one, so once the plan sends a student home the next trip sets off from home rather than from the classroom they have already left. `findContinuityBreaks` states that invariant as a check — leg *n* must start where leg *n−1* ended — and the planner turns any break into a visible warning. `buildTransitions` survives as a thin wrapper for the plain academic chain (empty `goHomeAfter`).
