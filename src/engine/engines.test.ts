...
import { formatClock, torontoDate, minutesBetween } from "@/time/toronto";
...
describe("home return", () => {
  ...
  it("matches the product example: ends 11:20, walk 12, next 13:00, buffer 5", () => {
    const a = analyzeHomeReturn({ gapStart: t(11, 20), nextClassStart: t(13), routeHome: walk(12), routeBack: walk(12) }, spec);
  ...
  it("with the default 10 min buffer the same gap gives 66 minutes", () => {
    const a = analyzeHomeReturn({ gapStart: t(11, 20), nextClassStart: t(13), routeHome: walk(12), routeBack: walk(12) }, CFG);
  ...
  it("barely enough -> POSSIBLE", () => {
    ...
    const a = analyzeHomeReturn({ gapStart: t(11), nextClassStart: t(12), routeHome: walk(12), routeBack: walk(12) }, CFG);
  ...
  it("not enough -> NOT_RECOMMENDED but still possible when usable > 0", () => {
    // gap 40 -> usable 6
    const a = analyzeHomeReturn({ gapStart: t(11), nextClassStart: t(11, 40), routeHome: walk(12), routeBack: walk(12) }, CFG);
    expect(a.usableHomeMinutes).toBe(6);
    expect(a.recommendation).toBe("NOT_RECOMMENDED");
    expect(a.possible).toBe(true);
  });
  it("back-to-back -> impossible, no times", () => {
    const a = analyzeHomeReturn({ gapStart: t(11), nextClassStart: t(11, 10), routeHome: walk(12), routeBack: walk(12) }, CFG);
    expect(a.usableHomeMinutes).toBe(-24);
  ...
  it("exact thresholds: 30 -> WORTH_IT, 29 -> POSSIBLE, 10 -> POSSIBLE, 9 -> NOT_RECOMMENDED", () => {
    const at = (usable: number) => analyzeHomeReturn({ gapStart: t(11), nextClassStart: t(11, usable + 12 + 12 + 10), routeHome: walk(12), routeBack: walk(12) }, CFG);
  ...
  it("thresholds are configurable", () => {
    const a = analyzeHomeReturn({ gapStart: t(11), nextClassStart: t(12), routeHome: walk(12), routeBack: walk(12) }, { ...CFG, minUsefulHomeMinutes: 20 });
    expect(a.recommendation).toBe("WORTH_IT");
  });
});
...
describe("transit comparison", () => {
  it("eligibility", () => {
    ...
    expect(shouldConsiderTransit(false, 12, CFG)).toBe(false);
    expect(shouldConsiderTransit(false, undefined, CFG)).toBe(false);
  });
